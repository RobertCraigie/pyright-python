...
NPX = str(node._ensure_available('npx').path)
...
def test_target_version(
    ...
    fake_process.register_subprocess([NPX, "--version"], stdout=output)
...
def test_target_version_not_found(
    ...
    fake_process.register_subprocess([NPX, "--version"], stdout='hello world')
